...
const useStyles = makeStyles({
  ...
  indicator: {
    ...
    '&::after': {
      ...
      // Removed transition, using animation instead
    },
  },
  activeIndicator: {
    '&::after': {
...
const HighlightCard = ({ icon, title, isActive, onClick }) => {
  ...
  // Reset animation key when isActive changes to force re-render
  useEffect(() => {
    if (isActive) {
    ...
    <Box className={`${classes.card} ${isActive ? classes.active : ''}`} onClick={onClick}>
      <Box className={classes.cardInner}>
        <Box className={classes.cardFront}>
          ...
          {/* Progress indicator with key to force reset */}
          <Box 
            key={animationKey} 
            className={`${classes.indicator} ${isActive ? classes.activeIndicator : ''}`}
          ></Box>
        </Box>
      </Box>
    </Box>
